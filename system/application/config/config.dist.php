...
|
*/
$config['uri_protocol']	= "PATH_INFO";  #Not AUTO or REQUEST_URI!
...
|
*/
$config['charset'] = "UTF-8";  # To match database.php
...
|
*/
//Bitbucket/ CloudEngine bug #60 - ensure '-' is permitted (eg. tags) - important: escape as \- .
$config['permitted_uri_chars'] = 'a-z 0-9~+%.:_\-АЮ?Ц???????Б?????ИХ???Й?????СР?У???????Т?????ЗЫ?????????МЛ???Щ?????аю?ц???????б?????их???й?????ср?у???????т?????зы?????????мл???щ?????#';
