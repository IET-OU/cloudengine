...
<?php if ($links): ?>
	<ul class="arrows">
	    <?php foreach($links as $link): ?>
	        <li class="cloud-link"><a name="link-<?=$link->link_id ?>" href="<?= $link->url ?>" class="<?php if ($link->type == 'cloud'):?> cloud<?php endif; ?><?php if ($link->type == 'cloudscape'):?> cloudscape<?php endif; ?>"><?= $link->title? $link->title : '<div style="word-wrap: break-word">'.$link->url.'</div>' ?></a>
